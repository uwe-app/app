...
impl HelperDef for Markdown {
    fn call<'reg: 'rc, 'rc>(
        ...
        let mut evaluate = h.hash_get("render")
            .map(|v| v.value())
            .or(Some(&json!(false)))
            ...
            if let Some(path_json) = h.param(0) {
                ...
                if path_json.is_value_missing() {
                    if let Some(ref path) = path_json.relative_path() {
                        let template = r.get_template(path)
                            .ok_or(
                                RenderError::new(
                                    format!("Type error for `md` helper, no template found for {}", path)
                                )
                            )?;
                        template.render(r, ctx, rc, &mut buf)?;
                    } else {
                        return Err(RenderError::new("Type error for `md` helper, unable to determine relative path"))
                    }
                } else {
                    let param = h.param(0)
                        .map(|v| v.value())
