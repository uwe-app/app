...
impl HelperDef for Feed<'_> {
    fn call<'reg: 'rc, 'rc>(
        ...
        let base_url = self.context.options.get_canonical_url(&self.context.config, true)
            .map_err(|_e| RenderError::new("Error in `feed` helper, failed to parse base URL"))?;

        let missing_feed = RenderError::new(
            &format!("Type error for `feed`, missing named feed {}", &name));
            ...
            if let Some(ref channel) = feed.channels.get(&name) {
                ...
                for feed_type in channel.types.iter() {
                    ...
                    let url = base_url.join(&path)
                        .map_err(|_e|
                            RenderError::new("Error in `feed` helper, failed to join URL"))?.to_string();
