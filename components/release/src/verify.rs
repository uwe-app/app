...
use crate::{Error, Result, releases, checksum};
...
pub(crate) fn test(version: &Version, names: &[&str]) -> Result<bool> {
    ...
    let info = releases.versions.get(version).ok_or_else(
        || Error::VersionNotFound(version.to_string()))?;

    let checksums = info.platforms.get(
        &releases::current_platform()).unwrap();
