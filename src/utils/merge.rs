...
pub fn map<T>(base: &T, from: &T) -> Result<T, Error> where T: Serialize + DeserializeOwned {
    ...
    if !base_val.is_object() || !from_val.is_object() {
        return Err(
            Error::new(
                format!("Must both be objects to merge")));
    }
    ...
    Ok(serde_json::from_value::<T>(serde_json::to_value(base_map)?)?)
}
