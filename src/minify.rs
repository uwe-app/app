...
// From: https://github.com/martingallagher/html5minify/blob/master/src/lib.rs
// Could not add as the dependency html5minify as it does not compile on stable
// rust due to the use of a feature macro that requires nightly
//
// Modified so that the logic is not so aggressive and does not trim 
// text nodes with content which causes problems when they have an adjacent
// inline element (spaces were removed).
//
// Added logic to escape text content otherwise HTML entities in the source 
// document are not respected.
...
impl<'a> Minifier<'a> {
    ...
    fn write_escaped(&mut self, text: &str, attr_mode: bool) -> io::Result<()> {
        for c in text.chars() {
            match c {
                '&' => self.w.write_all(b"&amp;"),
                '\u{00A0}' => self.w.write_all(b"&nbsp;"),
                '"' if attr_mode => self.w.write_all(b"&quot;"),
                '<' if !attr_mode => self.w.write_all(b"&lt;"),
                '>' if !attr_mode => self.w.write_all(b"&gt;"),
                c => self.w.write_fmt(format_args!("{}", c)),
            }?;
        }
        Ok(())
    }
        ...
        match node.data {
            NodeData::Text { ref contents } => {
                ...
                let contents = if trim {
                    is_blank = contents.chars().all(char::is_whitespace);
                ...
                if is_blank || contents.is_empty() {
                    return io::Result::Ok(());
                }

                self.write_escaped(contents, false)?

                //self.w.write_all(txt.as_bytes())?
            }
